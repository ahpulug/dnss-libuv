...
dns_record_t *dns_record_from_buf(buffer_t *const buffer, const size_t count)
{
    ...
    for(int i = 0; i < count; ++i)
    {
        ...
        record[i].qtype = buf_read_u16(buffer);

        switch(record[i].qtype)
        {
            case QTYPE_A:
                record[i].A.domain = domain;
                record[i].A.qclass = buf_read_u16(buffer);
                record[i].A.ttl = buf_read_u32(buffer);
                ...
                for(int p = 0; p < 4; ++p)
                {
                    record[i].A.addr[p] = buf_read_u8(buffer);
                }
                break;
            ...
            case QTYPE_CNAME:
                ...
                record[i].CNAME.qclass = buf_read_u16(buffer);
                record[i].CNAME.ttl = buf_read_u32(buffer);
                record[i].CNAME.data_len = buf_read_u16(buffer);
